import Effect from '../../models/move/effect'
...
export default class EffectParser {
  ...
  parseDefinition (pattern, definition) {
    ...
    const parser = { pattern, getEffect: params => definition.getValue(params) }
  ...
  parseUsage (rawEffect, params=[]) {
    ...
    if (!parser) {
      return new Effect.Text(rawEffect)
    }

    const params = this.patternParser.getParams(parser, rawEffect, params, (raw, { type, params }) => {
      if (type === 'options') {
        ...
        return this.context.fieldParser.existingFields().find(field => field.name === raw)
      } else if (type) {
...
const PRESET_PARSERS = [
  ...
  { pattern: 'modify <filter:filter> <formula:formula>', getEffect: ({ filter, fromula, once }) => new Effect.Modify(filter, formula) },
  { pattern: 'modify once <filter:filter> <formula:formula>', getEffect: ({ filter, fromula, once }) => new Effect.Modify(filter, formula, { once: true }) },
  ...
  { pattern: 'choose <string:text>', getEffect: ({ text }) => new Effect.Choice(text, outcomes) },
  { pattern: 'choose <number:count> <string:text>', getEffect: ({ text, count }) => new Effect.Choice(text, outcomes, count) },
  { pattern: 'choose <number:count>', getEffect: ({ count }) => new Effect.Choice('', outcomes, count) },
